    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        for (l = 0; l < 3; l++) {
            ...
            if (p == 0) {
                ...
                d.B0[ l + (i+0)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                if (l == 0)
                    detJacS[(p+0) + (q+1)*dimX         + (r+1)*dimX*dimY] =
                    d.detJac[ (i+0) + (j+1)*(dev_params.nx+2) + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)]; }
            if (((p == (blockDim.x-1)) && (i < (dev_params.nx-1))) || (i == (dev_params.nx-1))) {
                ...
                d.xb[ l + (i+2)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                 ...
                 d.B0[ l + (i+2)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                if (l == 0)
                    detJacS[(p+2) + (q+1)*dimX         + (r+1)*dimX*dimY] =
                    d.detJac[ (i+2) + (j+1)*(dev_params.nx+2) + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)]; }
            if (q == 0) {
                ...
                d.B0[ l + (i+1)*3      + (j+0)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                if (l == 0)
                    detJacS[(p+1) + (q+0)*dimX         + (r+1)*dimX*dimY] =
                    d.detJac[ (i+1) + (j+0)*(dev_params.nx+2) + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)]; }
            if (((q == (blockDim.y-1)) && (j < (dev_params.ny-1))) || (j == (dev_params.ny-1))) {
                ...
                d.B0[ l + (i+1)*3      + (j+2)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                if (l == 0)
                    detJacS[(p+1) + (q+2)*dimX         + (r+1)*dimX*dimY] =
                    d.detJac[ (i+1) + (j+2)*(dev_params.nx+2) + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)]; }
            if (r == 0) {
                ...
                d.B0[ l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+0)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                if (l == 0)
                    detJacS[(p+1) + (q+1)*dimX         + (r+0)*dimX*dimY] =
                    d.detJac[ (i+1) + (j+1)*(dev_params.nx+2) + (k+0)*(dev_params.nx+2)*(dev_params.ny+2)]; }
            if (((r == (blockDim.z-1)) && (k < (dev_params.nz-1))) || (k == (dev_params.nz-1))) {
                ...
                d.B0[ l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+2)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                if (l == 0)
                    detJacS[(p+1) + (q+1)*dimX         + (r+2)*dimX*dimY] =
                    d.detJac[ (i+1) + (j+1)*(dev_params.nx+2) + (k+2)*(dev_params.nx+2)*(dev_params.ny+2)]; }
        }
    }
    __syncthreads();
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        ...
        for (l = 0; l < 3; l++) {
            dB0_dX[l][0] = (B0s[l + (p+2)*3 + (q+1)*dimX*3  + (r+1)*dimX*dimY*3] -
                            B0s[l + (p+0)*3 + (q+1)*dimX*3  + (r+1)*dimX*dimY*3])*dev_params.dx1/2;
            dB0_dX[l][1] = (B0s[l + (p+1)*3 + (q+2)*dimX*3  + (r+1)*dimX*dimY*3] -
                            B0s[l + (p+1)*3 + (q+0)*dimX*3  + (r+1)*dimX*dimY*3])*dev_params.dy1/2;
            dB0_dX[l][2] = (B0s[l + (p+1)*3 + (q+1)*dimX*3  + (r+2)*dimX*dimY*3] -
                            B0s[l + (p+1)*3 + (q+1)*dimX*3  + (r+0)*dimX*dimY*3])*dev_params.dz1/2;
        }

        // Derivative of the inverse determinant of the Jacobian.
        dDet1_dX[0] = (1/detJacS[(p+2) + (q+1)*dimX  + (r+1)*dimX*dimY] -
                       1/detJacS[(p+0) + (q+1)*dimX  + (r+1)*dimX*dimY])*dev_params.dx1/2;
        dDet1_dX[1] = (1/detJacS[(p+1) + (q+2)*dimX  + (r+1)*dimX*dimY] -
                       1/detJacS[(p+1) + (q+0)*dimX  + (r+1)*dimX*dimY])*dev_params.dy1/2;
        dDet1_dX[2] = (1/detJacS[(p+1) + (q+1)*dimX  + (r+2)*dimX*dimY] -
                       1/detJacS[(p+1) + (q+1)*dimX  + (r+0)*dimX*dimY])*dev_params.dz1/2;

        // Add all up.
        d.J[0 + i*3 + j*dev_params.nx*3  + k*dev_params.nx*dev_params.ny*3] = 0;
        d.J[1 + i*3 + j*dev_params.nx*3  + k*dev_params.nx*dev_params.ny*3] = 0;
        d.J[2 + i*3 + j*dev_params.nx*3  + k*dev_params.nx*dev_params.ny*3] = 0;
        for (a = 0; a < 3; a++)
            for (b = 0; b < 3; b++) {
                d.J[0 + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3] +=
                ...
                d.J[1 + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3] +=
                ...
                d.J[2 + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3] +=
...
// ompute the electric current density using mimetic operators (Stokes method).
__global__ void JStokes(struct VarsDev d, int dimX, int dimY, int dimZ)
{
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        for (l = 0; l < 3; l++) {
            ...
            if (p == 0) {
                ...
                d.B[  l + (i+0)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]; }
            if (((p == (blockDim.x-1)) && (i < (dev_params.nx-1))) || (i == (dev_params.nx-1))) {
                xbs[l + (p+2)*3      + (q+1)*dimX*3          + (r+1)*dimX*dimY*3] =
                d.xb[ l + (i+2)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                 Bs[ l + (p+2)*3      + (q+1)*dimX*3          + (r+1)*dimX*dimY*3] =
                 d.B[  l + (i+2)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]; }
            if (q == 0) {
                xbs[l + (p+1)*3      + (q+0)*dimX*3          + (r+1)*dimX*dimY*3] =
                d.xb[ l + (i+1)*3      + (j+0)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                Bs[ l + (p+1)*3      + (q+0)*dimX*3          + (r+1)*dimX*dimY*3] =
                d.B[  l + (i+1)*3      + (j+0)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]; }
            if (((q == (blockDim.y-1)) && (j < (dev_params.ny-1))) || (j == (dev_params.ny-1))) {
                xbs[l + (p+1)*3      + (q+2)*dimX*3          + (r+1)*dimX*dimY*3] =
                d.xb[ l + (i+1)*3      + (j+2)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                Bs[ l + (p+1)*3      + (q+2)*dimX*3          + (r+1)*dimX*dimY*3] =
                d.B[  l + (i+1)*3      + (j+2)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]; }
            if (r == 0) {
                xbs[l + (p+1)*3      + (q+1)*dimX*3          + (r+0)*dimX*dimY*3] =
                d.xb[ l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+0)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                Bs[ l + (p+1)*3      + (q+1)*dimX*3          + (r+0)*dimX*dimY*3] =
                d.B[  l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+0)*(dev_params.nx+2)*(dev_params.ny+2)*3]; }
            if (((r == (blockDim.z-1)) && (k < (dev_params.nz-1))) || (k == (dev_params.nz-1))) {
                xbs[l + (p+1)*3      + (q+1)*dimX*3          + (r+2)*dimX*dimY*3] =
                d.xb[ l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+2)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                Bs[ l + (p+1)*3      + (q+1)*dimX*3          + (r+2)*dimX*dimY*3] =
                d.B[  l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+2)*(dev_params.nx+2)*(dev_params.ny+2)*3]; }
        }
    }
    __syncthreads();
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        ...
        for (l = 0; l < 3; l++) {
            ...
            d.J[l + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3] = 0;
            for (m = 0; m < 3; m++) {
                Js[l + p*3 + q*(dimX-2)*3 + r*(dimX-2)*(dimY-2)*3] += NA1[l][m]*II[m]; // area weighted
                d.J[l + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3] += NA1[l][m]*II[m];
            }
        }
    }
    __syncthreads();
...
__global__ void JStokes4th(struct VarsDev d, int dimX, int dimY, int dimZ)
{
    ...
    REAL I;         // electric current through a octilateral
    REAL NA[3][3];  // n.A with area weighted normals
    REAL NA1[3][3]; // inverse to NA
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        for (l = 0; l < 3; l++) {
            if ((((p == 0) && (q == 0)) || ((p == 0) && (q == (blockDim.y-1))) || ((p == (blockDim.x-1)) && (q == 0)) || ((p == (blockDim.x-1)) && (q == (blockDim.y-1)))
                ...
                d.B[  l + (i+2*u)*3    + (j+2*v)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            }
            if ((((p == 0) && (r == 0)) || ((p == 0) && (r == (blockDim.z-1))) || ((p == (blockDim.x-1)) && (r == 0)) || ((p == (blockDim.x-1)) && (r == (blockDim.z-1)))
                || ((i == 0) && (k == (dev_params.nz-1))) || ((i == (dev_params.nx-1)) && (k == 0)) || ((i == (dev_params.nx-1)) && (k == (dev_params.nz-1))))
                || (((p == 0) || (p == (blockDim.x-1))) && (k == (dev_params.nz-1))) || (((r == 0) || (r == (blockDim.z-1))) && (i == (dev_params.nx-1)))) {
                ...
                d.B[  l + (i+2*u)*3    + (j+1)*(dev_params.nx+2)*3    + (k+2*v)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            }
            if ((((r == 0) && (q == 0)) || ((r == 0) && (q == (blockDim.y-1))) || ((r == (blockDim.z-1)) && (q == 0)) || ((r == (blockDim.z-1)) && (q == (blockDim.y-1)))
                || ((j == 0) && (k == (dev_params.nz-1))) || ((j == (dev_params.ny-1)) && (k == 0)) || ((j == (dev_params.ny-1)) && (k == (dev_params.nz-1))))
                || (((r == 0) || (r == (blockDim.z-1))) && (j == (dev_params.ny-1))) || (((q == 0) || (q == (blockDim.y-1))) && (k == (dev_params.nz-1)))) {
                ...
                d.B[  l + (i+1)*3      + (j+2*u)*(dev_params.nx+2)*3  + (k+2*v)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            }
        }
    }
    __syncthreads();
    ...
    // Copy J into global memory.
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz))
       for (l = 0; l < 3; l++)
           d.J[l + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3] = Js[l + p*3 + q*(dimX-2)*3 + r*(dimX-2)*(dimY-2)*3];
}
...
__global__ void JStokesQuint(struct VarsDev d, int dimX, int dimY, int dimZ)
{
    ...
    REAL I;         // electric current through a quintilateral
    REAL detN;      // determinant of N
    REAL detN1;     // inverse determinant of N
    REAL NA[3][3];  // n.A with area weighted normals
    REAL NA1[3][3]; // inverse to NA
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        ...
        II[0] = I;
        // Compute the surface normal to this quintilateral, as in eq. (15), ref [1].
        for (l = 0; l < 3; l++) {
            NA[0][l] = (dx1Xdx2[l] + dx2Xdx3[l] + dx3Xdx4[l] + dx4Xdx1[l])/4;
            if (triangle > 0)
                NA[0][l] += dx5Xdx6[l]/2;
