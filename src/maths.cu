...
__global__ void mag(REAL *vec, REAL *mag) {
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz))
        mag[i + j*dev_params.nx + k*dev_params.nx*dev_params.ny] = norm(&vec[0 + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3]);
}
...
__global__ void JxB_B2(REAL *B, REAL *J, REAL *JxB_B2, int dimX, int dimY, int dimZ) {
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        for (l = 0; l < 3; l++) {
            Bs[l + p*3 + q*dimX*3 + r*dimX*dimY*3] = B[l + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            Js[l + p*3 + q*dimX*3 + r*dimX*dimY*3] = J[l + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3];
        }
        ...
        JxB_B2[i + j*dev_params.nx + k*dev_params.nx*dev_params.ny] = norm(&JxBs[0 + p*3 + q*dimX*3 + r*dimX*dimY*3])/B2;
    }
}
...
__global__ void JB_B2(REAL *B, REAL *J, REAL *JB_B2)
{
    ...
    // Indices for the most central location in the xy-plane.
    i = dev_params.nx/2;
    j = dev_params.ny/2;
    ...
    if (k < dev_params.nz) {
        for (l = 0; l < 3; l++) {
            Bl[l] = B[l + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            Jl[l] = J[l + i*3 + j*dev_params.nx*3 + k*dev_params.nx*dev_params.ny*3];
        }
...
__global__ void epsilonStar(REAL *xb, REAL *JB_B2, REAL *epsStar, int dimZ)
{
    ...
    i = dev_params.nx/2; j = dev_params.ny/2;

    iter = int(dev_params.nz-1)/dimZ + 1;

    // Copy from global to shared memory.
    if (k < dev_params.nz) {
        z[p]      = xb[2 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
        JB_B2s[p] = JB_B2[k];
    }

    for (m = 0; m < iter; m++) {
        if (k < dev_params.nz) {
            if (dimZ*m >= dev_params.nz) {
                z[p + dimZ]      = xb[2 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1+dimZ*m-dev_params.nz)*(dev_params.nx+2)*(dev_params.ny+2)*3];
                JB_B2s[p + dimZ] = JB_B2[k + dimZ*m - dev_params.nz];
            }
            else if (k + dimZ*m >= dev_params.nz) {
                z[p + dimZ]      = z[p];
                JB_B2s[p + dimZ] = JB_B2s[p];
            }
            else {
                z[p + dimZ]      = xb[2 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1+dimZ*m)*(dev_params.nx+2)*(dev_params.ny+2)*3];
        ...
        if (k < dev_params.nz)
            for (n = 0; n < dimZ; n++)
                if ((n + blockDim.x*m) < dev_params.nz)
                    epsStar[n + blockDim.x*m + dev_params.nz*k] = epsStarL[n];
    }
}
...
__global__ void B_1ez(REAL *B, REAL *B_1ez2)
{
    ...
    int p = threadIdx.x;
    int i = m%dev_params.nx;
    int k = m/(dev_params.nx*dev_params.ny);
    int j = m/(dev_params.nx) - k*dev_params.ny;
    int l;
    REAL sum;
    ...
    if (m < (dev_params.nx*dev_params.ny*dev_params.nz)) {
        B_1ezS[0 + 3*p] = B[0 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
        B_1ezS[1 + 3*p] = B[1 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
        B_1ezS[2 + 3*p] = B[2 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]-1.;
    }
    else {
        B_1ezS[0 + 3*p] = 0;
        B_1ezS[1 + 3*p] = 0;
...
__global__ void xb_XbAn(REAL *xb, REAL *xb_xbAn)
{
    ...
    x = i*dev_params.dx + dev_params.Ox;
    y = j*dev_params.dy + dev_params.Oy;
    z = k*dev_params.dz + dev_params.Oz;

    if (m < (dev_params.nx*dev_params.ny*dev_params.nz)) {
        xb_xbAnS[0 + 3*p] = xb[0 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]
                               - cos(exp(-(x*x+y*y)/(dev_params.ar*dev_params.ar)-z*z/(dev_params.az*dev_params.az))*dev_params.phi1)*x
                               - sin(exp(-(x*x+y*y)/(dev_params.ar*dev_params.ar)-z*z/(dev_params.az*dev_params.az))*dev_params.phi1)*y;
        xb_xbAnS[1 + 3*p] = xb[1 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]
                               + sin(exp(-(x*x+y*y)/(dev_params.ar*dev_params.ar)-z*z/(dev_params.az*dev_params.az))*dev_params.phi1)*x
                               - cos(exp(-(x*x+y*y)/(dev_params.ar*dev_params.ar)-z*z/(dev_params.az*dev_params.az))*dev_params.phi1)*y;
        xb_xbAnS[2 + 3*p] = xb[2 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3] - z;
    }
    else {
        xb_xbAnS[0 + 3*p] = 0;
        xb_xbAnS[1 + 3*p] = 0;
...
__global__ void dirCellVol(REAL *xb, REAL *cellVol, int dimX, int dimY, int dimZ)
{
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        for (l = 0; l < 3; l++) {
            ...
            xb[ l + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            ...
            if (((p == (blockDim.x-1)) && (i < (dev_params.nx-1))) || (i == (dev_params.nx-1)))
                ...
                xb[ l + (i+2)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            if (((q == (blockDim.y-1)) && (j < (dev_params.ny-1))) || (j == (dev_params.ny-1)))
                xbs[l + (p+0)*3      + (q+1)*dimX*3          + (r+0)*dimX*dimY*3] =
                xb[ l + (i+1)*3      + (j+2)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            if (((r == (blockDim.z-1)) && (k < (dev_params.nz-1))) || (k == (dev_params.nz-1)))
                xbs[l + (p+0)*3      + (q+0)*dimX*3          + (r+1)*dimX*dimY*3] =
                xb[ l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+2)*(dev_params.nx+2)*(dev_params.ny+2)*3];
        }
    }
    __syncthreads();
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
       ...
       cellVol[i + j*dev_params.nx + k*dev_params.nx*dev_params.ny] = cellVolS[p + q*(dimX-1) + r*(dimX-1)*(dimY-1)];
    }
}
...
__global__ void gridWedgeMin(REAL *xb, REAL *wedge, int dimX, int dimY, int dimZ)
{
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        for (l = 0; l < 3; l++) {
            ...
            if (p == 0)
                ...
                xb[ l + (i+0)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            if (((p == (blockDim.x-1)) && (i < (dev_params.nx-1))) || (i == (dev_params.nx-1)))
                xbs[l + (p+2)*3      + (q+1)*dimX*3          + (r+1)*dimX*dimY*3] =
                xb[ l + (i+2)*3      + (j+1)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            if (q == 0)
                xbs[l + (p+1)*3      + (q+0)*dimX*3          + (r+1)*dimX*dimY*3] =
                xb[ l + (i+1)*3      + (j+0)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            if (((q == (blockDim.y-1)) && (j < (dev_params.ny-1))) || (j == (dev_params.ny-1)))
                xbs[l + (p+1)*3      + (q+2)*dimX*3          + (r+1)*dimX*dimY*3] =
                xb[ l + (i+1)*3      + (j+2)*(dev_params.nx+2)*3  + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            if (r == 0)
                xbs[l + (p+1)*3      + (q+1)*dimX*3          + (r+0)*dimX*dimY*3] =
                xb[ l + (i+1)*3      + (j+1)*(dev_params.nx+2)*3  + (k+0)*(dev_params.nx+2)*(dev_params.ny+2)*3];
            if (((r == (blockDim.z-1)) && (k < (dev_params.nz-1))) || (k == (dev_params.nz-1)))
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        ...
        wedge[i + j*dev_params.nx + k*dev_params.nx*dev_params.ny] = wedgeMin;
    }
}
...
__global__ void gridConvexity(REAL *xb, REAL *convexity, int dimX, int dimY, int dimZ)
{
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        ...
        // Copy back to global memory.
        convexity[i + j*dev_params.nx + k*dev_params.nx*dev_params.ny] = convexityS[p + q*(dimX-2) + r*(dimX-2)*(dimY-2)];
    }
}
...
__global__ void B2_det(REAL *B, REAL *detJac, REAL *B2_det)
{
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        B2_detL = dot(&B[ 0 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3],
                      &B[ 0 + (i+1)*3 + (j+1)*(dev_params.nx+2)*3 + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)*3]) *
                  detJac[     (i+1)   + (j+1)*(dev_params.nx+2)   + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)];
        // Account faces, edges and corners to only 1/2, 1/4 and 1/8.
        bound = (i == 0) + (j == 0) + (k == 0) + (i == (dev_params.nx-1)) + (j == (dev_params.ny-1)) + (k == (dev_params.nz-1));
        bound = 1 << bound;
        B2_detL /= bound;
    }
    else
...
__global__ void U2_det(REAL *uu, REAL *detJac, REAL *U2_det)
{
    ...
    if ((i < dev_params.nx) && (j < dev_params.ny) && (k < dev_params.nz)) {
        U2_detL = dot(&uu[ 0 + i*3   + j*dev_params.nx*3       + k*dev_params.nx*dev_params.ny*3],
                      &uu[ 0 + i*3   + j*dev_params.nx*3       + k*dev_params.nx*dev_params.ny*3]) *
                  detJac[      (i+1) + (j+1)*(dev_params.nx+2) + (k+1)*(dev_params.nx+2)*(dev_params.ny+2)];
        // Account faces, edges and corners to only 1/2, 1/4 and 1/8.
        bound = (i == 0) + (j == 0) + (k == 0) + (i == (dev_params.nx-1)) + (j == (dev_params.ny-1)) + (k == (dev_params.nz-1));
        bound = 1 << bound;
