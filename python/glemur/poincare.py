...
class poincareMap:
    ...
    def __init__(self, dataDir = 'data', B0File = 'B0.vtk', poincare = 'poincareInit.vtk', paramFile = 'save.vtk', dumpFile = [], interpolation = 'weighted', integration = 'simple', hMin = 2e-3, hMax = 2e2, lMax = 500, tol = 2e-3, iterMax = 1e3, x0 = [], y0 = [], nSeeds = 10, nIter = 10):
        ...
        if (poincare != []):
            ...
            tmp[5] = np.array([s.tol], dtype = 'int32'); field = VN.numpy_to_vtk(tmp[5]); field.SetName('nSeeds'); fieldData.AddArray(field)
            tmp[6] = np.array([s.tol], dtype = 'int32'); field = VN.numpy_to_vtk(tmp[6]); field.SetName('nIter'); fieldData.AddArray(field)
